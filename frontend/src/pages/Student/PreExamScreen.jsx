...
export default function PreExamScreen() {
  ...
  const { data: examData, isLoading, error } = useQuery({
    ...
    queryFn: () => studentExamService.getExamDetails(examId),
    retry: 1,
  });

  console.log('Exam Data:', examData);

  // Start exam mutation - FIXED
  const startMutation = useMutation({
    mutationFn: () => studentExamService.startExam(examId, selfieFile),
    onSuccess: (response) => {
      console.log('Start Exam Response:', response);
      
      // The correct path based on your API response
      const attemptId = response?.data?.exam_attempt_id;
      
      console.log('Attempt ID:', attemptId);
      
      if (attemptId) {
        navigate(`/student/exam/${attemptId}/session`);
      } else {
        console.error('Could not find exam_attempt_id in response:', response);
        alert('Exam started but navigation failed. Please check your active exams.');
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">
            {error.response?.data?.message || "The exam you're looking for doesn't exist or has been removed."}
          </p>
          <button
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="max-w-md w-full bg-white rounded-lg shadow-lg p-8 text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">No Exam Data</h2>
          <p className="text-gray-600 mb-6">
            Unable to load exam information. Please try again.
          </p>
          <button
            onClick={() => navigate('/student/dashboard')}
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <div className="text-center p-4 bg-green-50 rounded-lg">
              ...
              <p className="text-xl font-bold text-gray-900">{exam.total_questions || 0}</p>
            </div>
          ...
          <div className="mt-4 p-3 bg-blue-50 rounded-lg">
            <p className="text-sm text-blue-800">
              <strong>Attempts:</strong> {exam.attempts_taken || 0} of {exam.max_attempts} used
            </p>
          </div>
        </div>
        ...
        <div className="text-center">
          <button
            ...
            disabled={!agreedToTerms || startMutation.isPending || (exam.require_webcam && !selfieFile) || !exam.can_attempt}
